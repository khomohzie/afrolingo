...
// --- Sub-Component: Accuracy Visualizer ---
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center gap-3 w-full">
      <div className="relative w-24 h-24">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-2xl font-black leading-none ${textColorClass}`}>
            {score}%
          </span>
        </div>
      </div>
...
export default function PracticePage() {
  ...
  // Audio context refs for real-time visualization
  const audioContextRef = useRef<AudioContext | null>(null);
  const analyserRef = useRef<AnalyserNode | null>(null);
  const animationFrameRef = useRef<number | null>(null);
  const idleAnimationRef = useRef<number | null>(null);

  const cacheKey = `edge-tts-${CURRENT_PHRASE.targetPhrase}`;
      ...
      const animateIdle = () => {
        setWaveHeights((prev) =>
          prev.map((h) => {
            ...
            return Math.min(80, Math.max(15, h + delta));
          })
        );
        idleAnimationRef.current = window.setTimeout(animateIdle, 150) as unknown as number;
      };
      animateIdle();
    }
  ...
  // --- Preload Edge TTS for Target Phrase ---
  useEffect(() => {
  ...
  const response = await fetch("/api/edge-tts", {
    ...
    body: JSON.stringify({ text: CURRENT_PHRASE.targetPhrase, }), // use your text variable here
  });

  if (!response.ok) {
     console.warn("Audio preload failed, but continuing...");
     return; // Stop here without crashing the page
  }
  ...
  console.error("Silent audio error:", error);
}
    };
    preloadAudio();
  }, [cacheKey]);

  // --- Interaction: AI Edge TTS Feedback ---
  const handleAiResponse = useCallback(async (feedbackText: string) => {
    ...
    try {
      const response = await fetch("/api/edge-tts", {
        ...
        body: JSON.stringify({
          text: feedbackText,
          voice: "en-US-AvaNeural", // Feedback in English
        }),
      });

      if (!response.ok) throw new Error("Edge TTS fetch failed");
      ...
      mediaRecorderRef.current.onstop = () => {
        ...
        reader.onloadend = async () => {
          try {
            ...
            if (data.feedback) {
              setAccuracyScore(data.score ?? 75);
  ...
  // --- Interaction: Play Native Yoruba Audio ---
  const handlePlayNative = useCallback(async () => {
    ...
    if (cachedUrl) {
      ...
      audio.play().catch(() => setIsPlayingNative(false));
      return;
    }
      ...
      <div className="min-h-screen flex flex-col bg-surface font-sans text-foreground">
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-4xl overflow-hidden float-shadow mb-8">
            ...
            <div className="animate-panel-left bg-surface-container-lowest p-10 md:p-14 flex flex-col justify-center">
              ...
              <div className="flex items-center gap-4 mb-16">
                <Button
                  ...
                  className="flex items-center gap-3 px-6 h-14 text-lg rounded-xl transition-colors bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-70"
                >
                  {isPreloading ? (
            ...
            {/* Right Panel: The Recording Interaction */}
            <div className="animate-panel-right bg-surface-container-low p-10 md:p-14 flex flex-col items-center justify-center relative min-h-125">
              ...
              <div className="flex items-center justify-center gap-1.5 h-32 mb-16">
                {waveHeights.map((height, i) => (
                  <div
                    key={i}
                    className={`w-2 rounded-full transition-all duration-100 ${isRecording || analysisStatus === "aiSpeaking"
                        ? "bg-primary"
                        : "bg-primary/30"
                      }`}
                    style={{ height: `${height}%` }}
              ...
              <div className="mt-8 text-center flex flex-col items-center justify-start w-full max-w-sm gap-4">
                ...
                {analysisStatus === "idle" && !aiSubtitle && accuracyScore === null && (
