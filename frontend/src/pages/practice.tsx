...
const DEFAULT_WAVE_HEIGHTS = [40, 60, 30, 80, 100, 60, 40, 90, 70, 50, 80, 40, 60];
...
function AccuracyRing({ score }: { score: number }) {
    ...
    const colorClass =
        score >= 80 ? "stroke-green-500" : score >= 60 ? "stroke-amber-400" : "stroke-red-400";
    const textColorClass =
        score >= 80 ? "text-green-500" : score >= 60 ? "text-amber-400" : "text-red-400";
    const label =
        score >= 80
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center gap-3 w-full">
            <div className="relative w-24 h-24">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-2xl font-black leading-none ${textColorClass}`}>{score}%</span>
                </div>
            </div>
...
export default function PracticePage() {
    ...
    useEffect(() => {
        const shouldAnimate = !isRecording && analysisStatus !== "aiSpeaking" && analysisStatus !== "analyzing";
        ...
        utterance.onstart = () => {
            ...
            const updateWave = () => {
                if (analyserRef.current) {
                    ...
                    setWaveHeights(
                        Array.from(dataArray).slice(0, 13).map(val => Math.max(20, (val / 255) * 100))
                    );
                        ...
                        <div className="animate-panel-left bg-surface-container-lowest p-10 md:p-14 flex flex-col justify-center">
                            ...
                            <div className="flex items-center gap-4 mb-16">
                                <Button
                                    ...
                                    className={`flex items-center gap-3 px-6 h-14 text-lg rounded-xl transition-colors ${isPlayingNative || isPreloading
                                ...
                                >
                                    ...
                                    {isPreloading ? "Loading audio..." : isPlayingNative ? "Speaking..." : "Listen to Native"}
                                </Button>
                            </div>
