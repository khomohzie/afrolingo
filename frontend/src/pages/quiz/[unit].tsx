...
import { Trophy, X, Volume2, Loader2 } from "lucide-react";
...
export default function QuizPage() {
        ...
        const fetchQuiz = async () => {
            try {
                ...
                if (json.success) {
                    ...
                    console.error("Failed to load quiz", json.message);
                }
            } catch (error) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-background text-primary">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <h2 className="font-heading font-bold text-xl">Loading your lesson...</h2>
                </div>
            </div>
        );
    }
    ...
    const handleNextOrSubmit = async () => {
        ...
        const newAnswers = [
            ...userAnswers,
            { phraseId: currentQ.phraseId, answer: selectedOption.toLowerCase() } // API expects lower case string for answer
        ];
        ...
        if (currentIndex < quizData.totalQuestions - 1) {
            ...
            try {
                ...
                const json = await res.json();
                if (json.success) {
                    setQuizResults(json.data); // This triggers the Bridge Screen
                } else {
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6">
                <div className="max-w-md w-full text-center space-y-8 animate-in zoom-in duration-500">
                    ...
                    <div className={`w-32 h-32 rounded-full flex items-center justify-center mx-auto shadow-xl ${quizResults.passed ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'}`}>
                    ...
                    <div>
                        ...
                        <p className="text-lg text-on-surface-variant">
                            You scored {quizResults.percentage}% ({quizResults.correct}/{quizResults.total} correct). You earned {quizResults.xpEarned} XP!
                        </p>
                    </div>
                    ...
                    {quizResults.passed && (
                        <div className="p-6 bg-surface-container-lowest border border-border rounded-2xl flex items-center gap-4 text-left mt-8">
                            ...
                            <div>
                                <h3 className="font-bold text-foreground">Next Up: AI Voice Lab</h3>
                                <p className="text-sm text-muted-foreground">Perfect your {quizData.language} tones.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <Button
                        ...
                        onClick={() => quizResults.passed ? router.push('/practice') : router.reload()}
                    >
        ...
        <>
            ...
            <div className="min-h-screen flex flex-col bg-background text-foreground">
                ...
                <header className="w-full max-w-4xl mx-auto p-6 flex items-center gap-6">
                    <Link href="/learn" className="text-on-surface-variant hover:text-primary transition-colors">
                    ...
                    <div ref={questionContainerRef} className="w-full flex flex-col items-center">
                        ...
                        {currentQ.type === "listen_pick" && (
                            <button
                                ...
                                className={`w-24 h-24 mb-10 rounded-full flex items-center justify-center shadow-xl transition-all ${isPlayingAudio
                                ...
                                {currentQ.options.map((option, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-6 rounded-2xl text-lg font-bold text-center transition-all duration-200 active:scale-95 ${isSelected
                ...
                <footer className="fixed bottom-0 left-0 w-full border-t border-border bg-background">
                    <div className="max-w-4xl mx-auto px-6 py-6 sm:py-8 flex flex-col sm:flex-row items-center justify-end gap-4">
                        <Button
                            ...
                            className={`w-full sm:w-auto min-w-[180px] h-14 text-lg font-bold rounded-2xl transition-all ${selectedOption
                                ? "bg-primary text-on-primary hover:bg-primary/90 hover:scale-105"
                                : "bg-surface-variant text-on-surface-variant cursor-not-allowed"
                                }`}
                        >
                            {isSubmitting ? <Loader2 className="animate-spin" /> : currentIndex === quizData.totalQuestions - 1 ? "SUBMIT QUIZ" : "NEXT QUESTION"}
                        </Button>
                    </div>
                </footer>
